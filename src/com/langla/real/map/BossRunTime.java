                    ...
                    for (int i = 0; i < listBoss.size(); i++) {
                        ...
                        if (boss.timeDelay < System.currentTimeMillis()) {
                            if (boss.isDie) {
                                ...
                                boss.isDie = false;
                                UTPKoolVN.Print("Create Boss: " + boss.name + " Map: " + boss.map + " Zone: "
                                        + zonerandom + " ");
                                PlayerManager.getInstance().chatWord(
                                        "Boss " + boss.name + " vừa xuất hiện tại " + map.getMapTemplate().name);
                            } else {
                                Map map = Map.maps[boss.map];
                                boss.timeDelay = System.currentTimeMillis() + (boss.min_spam * 60000L);
                                PlayerManager.getInstance().chatWord(
                                        "Boss " + boss.name + " đã xuất hiện tại  " + map.getMapTemplate().name);
                            }
                        }
                    }
                    try {
