...
public class Giftcode {
    public static void useCode(String code, Client client) {
        ...
        if (usedCode(code, client.mChar.id)) {
            ...
            for (GiftcodeTemplate gift : datacode) {
                ...
                // thu.item = itemAdd;
                // if (itemAdd.expiry > 0)
                // thu.item.expiry = System.currentTimeMillis() + itemAdd.expiry;
                // }
    ...
    // if (itemThu.getItemTemplate().id == 789 || itemThu.getItemTemplate().id ==
    // 880) {
    // itemThu.strOptions = "";
    ...
    // List<Integer> optionRandom = DataCache.OptionCaiTrangVIP
    // .get(Utlis.nextInt(DataCache.OptionCaiTrangVIP.size()));

    // int id = optionRandom.get(0);
    // int param = Utlis.nextInt(optionRandom.get(1), optionRandom.get(2));

    // List<Integer> optionRandom2 = DataCache.OptionCaiTrangVIP
    // .get(Utlis.nextInt(DataCache.OptionCaiTrangVIP.size()));
    ...
    public static List<GiftcodeTemplate> checkCodeFormData(String code, Client client) {
        ...
        try (Connection con = PKoolVN.getConnection();
            ...
            if (red != null && red.first()) {
                ...
                return mapper.readValue(red.getString("list_item"),
                        TypeFactory.defaultInstance().constructCollectionType(List.class, GiftcodeTemplate.class));
            } else {
                return null;
            }
    ...
    // String updateQuery = "UPDATE `giftcode` SET count = count - 1 WHERE `code` =
    // ? AND count > 0";
    // String insertQuery = "INSERT INTO `giftcode_used` (`code`, `character_id`,
    // `time_used`) VALUES (?, ?, CURRENT_TIMESTAMP)";
    //
